package mate.global.login.filter;
...
public class CustomJsonUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    private static final String KAKAO_LOGIN_URL = "/user/kakaologin"; // "/user/kakaologin" 요청을 처리
    ...
    private static final AntPathRequestMatcher KAKAO_LOGIN_PATH_REQUEST_MATCHER =
            new AntPathRequestMatcher(KAKAO_LOGIN_URL, HTTP_METHOD); // "/user/kakaologin" + POST로 온 요청에 매칭

    private final ObjectMapper objectMapper;


    public CustomJsonUsernamePasswordAuthenticationFilter(ObjectMapper objectMapper) {
//        super(KAKAO_LOGIN_PATH_REQUEST_MATCHER); // 위에서 설정한 "login" + POST로 온 요청을 처리하기 위해 설정
        super(new OrRequestMatcher(KAKAO_LOGIN_PATH_REQUEST_MATCHER, DEFAULT_LOGIN_PATH_REQUEST_MATCHER));
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException {
        ...
        if (DEFAULT_LOGIN_PATH_REQUEST_MATCHER.matches(request)) {
            password = usernamePasswordMap.get(PASSWORD_KEY);
        } else if (KAKAO_LOGIN_PATH_REQUEST_MATCHER.matches(request)) {
            password = "kakao";
        } else {
