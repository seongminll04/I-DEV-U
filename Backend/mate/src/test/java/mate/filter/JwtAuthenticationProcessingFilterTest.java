...
class JwtAuthenticationProcessingFilterTest {
    ...
    private void init() {
        userRepository.save(User.builder().email(EMAIL).password(delegatingPasswordEncoder.encode(PASSWORD))
                .nickname("KSH1").role(Role.USER).gender(UserGender.MALE).status(UserStatus.A).build());
        clear();
    }
    ...
    void Access_Refresh_not_exist() throws Exception {
        // when, then
        mockMvc.perform(get("/user/sleep")) // "/login"이 아니고, 존재하는 주소를 보내기
    ...
    void Access_expired_and_Refresh_valid() throws Exception {
        ...
        // when, then
        MvcResult result = mockMvc.perform(get("/user/sleep") // "/login"이 아니고, 존재하는 주소를 보내기
